...
def render_chat():
    ...
    if notif_enabled or sound_enabled:
        ...
        if last_msg and last_msg.get("user_id") != st.session_state.user_id:
            ...
            play_sound = "true" if sound_enabled else "false"
            notif_on = "true" if notif_enabled else "false"
            components.html(
                f"""
                <script>
                (function() {{
                    ...
                    if ({notif_on} && w.Notification && w.Notification.permission === "granted" && w.document.hidden) {{
                        try {{
